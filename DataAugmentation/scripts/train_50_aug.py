...
IN_AUG_50_MNIST = "resize-dataset/data_aug_50_downscale"
...
class DataSet:
  ...
  def _filter_digits(self):
    mask = [True if label in DIGITS else False for label in self.labels]
...
print(f"Train error: {np.count_nonzero(train_missclassified)}")
...
df.to_csv("miss_subset_50_param_02_aug.csv", index=False)
